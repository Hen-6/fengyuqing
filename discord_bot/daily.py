...
RANK_PATH = Path(__file__).parent.parent / "data" / "rank.json"
YXCS_URLS = [
                ...
                async with session.get(url, headers={"User-Agent": "fengyuqing-discord/1.0"}) as resp:
                    ...
                    for raw_line in text.split("\n"):
                        ...
                        if alt_id not in _poem_cache:
                            _poem_cache[alt_id] = _poem_cache[pid]

    await asyncio.gather(*[fetch_file(url) for url in YXCS_URLS])
    _loaded = True
    print(f"[daily] 已加载 {len(_poem_cache)} 首诗词到缓存")
...
def find_poem_by_title(title: str) -> Poem | None:
    """按标题查找诗词"""
    return next(
        (p for p in _poem_cache.values() if p.name == title),
        None
    )
...
async def on_ready():
    ...
    print("[bot] 正在后台加载诗词数据...")
    await _load_poems()
    print(f"[bot] 启动完毕，共 {len(_poem_cache)} 首诗词")
...
async def cmd_today(interaction: discord.Interaction):
    ...
    if not poem:
        await interaction.followup.send(
            f"❌ 未找到《{entry['t']}》的诗词内容，请稍后重试", ephemeral=True
        )
        return
...
async def cmd_review(interaction: discord.Interaction):
    ...
    due = [
        ...
        if poems_data.get(f"{e['t']}:{e['a']}", {}).get("nextReview", "") <= today
    ]
    ...
    embed = poem_to_embed(
        ...
        show_note=True,
        extra_fields=[
            ("熟练度", f"当前 Lv.{poems_data.get(entry['t'] + ':' + entry['a'], {}).get('level', '?')}")
        ],
        color=GREEN,
    )
    await interaction.followup.send(embed=embed)
...
@app_commands.describe(mode="模式：同诗 或 跨诗（留空默认跨诗）")
async def cmd_jielong(interaction: discord.Interaction, mode: str = "跨诗"):
    ...
    poem = results[0].poem
    line = next(
        (l for l in poem.content if len(strip_punct(l)) >= 4),
        poem.content[0] if poem.content else "",
    )
...
async def cmd_verify(interaction: discord.Interaction, line: str):
    ...
    pid = f"{poem.name}:{poem.author}"
    if pid not in poems_data or (poems_data[pid].get("level", 0) < 3):
        poems_data[pid] = {
            "level": 3,
            "halfLifeDays": 1,
    ...
    embed = poem_to_embed(poem, f"{msg} 已记录为 Lv.3", show_note=True, color=GREEN)
    await interaction.followup.send(embed=embed)
